...
def test_create_resnet_and_forward_features() -> None:
    ...
    variables = resnet.init(
        jax.random.PRNGKey(1), jnp.ones((1, 48, 48, 1)), train=False
    )
...
def test_build_finetune_mask_variants() -> None:
    ...
    params = resnet.init(
        jax.random.PRNGKey(2), jnp.ones((1, 48, 48, 1)), train=False
    )["params"]

    no_freeze_mask = model.build_finetune_mask(
        {"params": params}, config=resnet.config
    )
    ...
    assert not np.any(
        np.asarray(mask["params"]["stage1_block1"]["conv1"]["kernel"])
    )
    assert np.all(
        np.asarray(mask["params"]["stage3_block1"]["conv1"]["kernel"])
    )
...
def test_maybe_load_pretrained_params_roundtrip(tmp_path: Path) -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(
        jax.random.PRNGKey(3), jnp.ones((1, 48, 48, 1)), train=False
    )["params"]
    ...
    checkpointer.save(
        str(checkpoint_path), params, save_args=save_args, force=True
    )
...
def test_bottleneck_projection_paths() -> None:
    ...
    key = jax.random.PRNGKey(0)
    block = model.BottleneckBlock(
        features=32, strides=(2, 2), use_projection=True
    )
    x = jnp.ones((1, 8, 8, 16))
...
def test_resnet_input_projection_branch() -> None:
    ...
    resnet = model.create_resnet(
        depth=18, num_classes=2, input_projection_channels=3
    )
    variables = resnet.init(
        jax.random.PRNGKey(9), jnp.ones((1, 48, 48, 3)), train=False
    )
...
def test_resnet_dropout_applies_when_requested() -> None:
    ...
    base = model.create_resnet(depth=18, num_classes=2)
    resnet = model.ResNet(
        config=base.config, include_top=True, dropout_rate=0.5
    )
    variables = resnet.init(
        jax.random.PRNGKey(4), jnp.ones((1, 48, 48, 1)), train=True
    )
...
def test_build_finetune_mask_without_container() -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(
        jax.random.PRNGKey(6), jnp.ones((1, 48, 48, 1)), train=False
    )["params"]
    unfrozen = unfreeze(params)
    mask = model.build_finetune_mask(
        unfrozen, config=resnet.config, freeze_classifier=True
    )
...
def test_maybe_load_pretrained_params_no_checkpoint() -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(
        jax.random.PRNGKey(7), jnp.ones((1, 48, 48, 1)), train=False
    )["params"]
    assert (
        model.maybe_load_pretrained_params(params, config=resnet.config)
        is params
    )
...
def test_build_finetune_mask_with_container() -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(
        jax.random.PRNGKey(11), jnp.ones((1, 48, 48, 1)), train=False
    )["params"]
...
def test_maybe_load_pretrained_params_freezes_restored(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        model.ocp, "PyTreeCheckpointer", lambda: DummyCheckpointer()
    )
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(
        jax.random.PRNGKey(8), jnp.ones((1, 48, 48, 1)), train=False
    )["params"]
    restored = model.maybe_load_pretrained_params(
        params,
...
def test_maybe_load_pretrained_params_handles_non_frozendict(
    monkeypatch,
) -> None:
    class DummyCheckpointer:
        def restore(self, path: str, item, restore_args=None):
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(
        jax.random.PRNGKey(9), jnp.ones((1, 48, 48, 1)), train=False
    )["params"]
    restored = model.maybe_load_pretrained_params(
        params,
        config=replace(resnet.config, checkpoint_path=Path("dummy")),
...
def test_maybe_load_pretrained_params_returns_frozendict(monkeypatch) -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(
        jax.random.PRNGKey(10), jnp.ones((1, 48, 48, 1)), train=False
    )["params"]
    restored = model.maybe_load_pretrained_params(
        params,
        config=replace(resnet.config, checkpoint_path=Path("dummy")),
