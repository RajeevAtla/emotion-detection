...
def test_build_finetune_mask_variants() -> None:
    ...
    params = resnet.init(jax.random.PRNGKey(2), jnp.ones((1, 48, 48, 1)), train=False)[
        "params"
    ]
    ...
    cfg = replace(
        resnet.config, freeze_stem=True, freeze_classifier=True, frozen_stages=(1,)
    )
...
def test_maybe_load_pretrained_params_roundtrip(tmp_path: Path) -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(jax.random.PRNGKey(3), jnp.ones((1, 48, 48, 1)), train=False)[
        "params"
    ]
...
def test_build_finetune_mask_without_container() -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(jax.random.PRNGKey(6), jnp.ones((1, 48, 48, 1)), train=False)[
        "params"
    ]
...
def test_maybe_load_pretrained_params_no_checkpoint() -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(jax.random.PRNGKey(7), jnp.ones((1, 48, 48, 1)), train=False)[
        "params"
    ]
...
def test_build_finetune_mask_with_container() -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(jax.random.PRNGKey(11), jnp.ones((1, 48, 48, 1)), train=False)[
        "params"
    ]
...
def test_maybe_load_pretrained_params_freezes_restored(monkeypatch) -> None:
    ...
    resnet = model.create_resnet(depth=18, num_classes=2)
    params = resnet.init(jax.random.PRNGKey(8), jnp.ones((1, 48, 48, 1)), train=False)[
        "params"
    ]
    restored = model.maybe_load_pretrained_params(
        params,
