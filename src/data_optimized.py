"""Optimized data loading with caching and prefetching for emotion detection."""
...
def _load_image(path: Path) -> np.ndarray:
    """Load an image from disk as uint8 array with shape (H, W, 1)."""
...
class ImageCache:
    """Thread-safe image cache for faster data loading."""

    def __init__(self, max_size_mb: int = 4096):
        """Initialize the cache with a maximum size in MB."""
    ...
    def get(self, path: Path) -> Optional[np.ndarray]:
        """Get an image from cache if present."""
        with self._lock:
            return self._cache.get(path)

    def put(self, path: Path, image: np.ndarray) -> None:
        """Add an image to cache if there's space."""
    ...
    def preload(
        ...
        """Preload images into cache using multiple threads."""
            ...
            for path, image in executor.map(load_one, paths_to_load):
                if image is not None:
                    ...
                    if show_progress and loaded % 1000 == 0:
                        print(f"  Cached {loaded}/{len(paths_to_load)} images", flush=True)

        if show_progress:
            print(f"  Cached {loaded} images ({self._current_bytes / 1024 / 1024:.1f} MB)", flush=True)
...
class BatchPrefetcher:
    ...
    def __init__(
        self,
        batch_iterator: Iterator[Tuple[jnp.ndarray, jnp.ndarray]],
        prefetch_count: int = 2,
    ):
        """Initialize the prefetcher."""
        self._iterator = batch_iterator
...
class EmotionDataModuleOptimized:
    """Optimized data module with caching and prefetching."""

    def __init__(self, config: DataModuleConfig) -> None:
        """Initialize the data module with its configuration."""
        self.config = config
        ...
        self._cache: Optional[ImageCache] = None if not config.cache_images else ImageCache()
    ...
    def setup(self, force_recompute_stats: bool = False) -> None:
        """Initialize dataset splits, statistics, and class weights."""
        cfg = self.config
        ...
        if self._cache is not None:
            print("Preloading training images into cache...", flush=True)
            all_paths = (
                [s.path for s in self._train_samples]
                + [s.path for s in self._val_samples]
                + [s.path for s in self._test_samples]
            )
            self._cache.preload(all_paths, num_workers=cfg.num_workers)
    ...
    def train_batches(
        ...
        *,
        ...
        if prefetch and self.config.prefetch_batches > 0:
            return BatchPrefetcher(iterator, self.config.prefetch_batches)
        return iterator
    ...
    def _load_image_cached(self, path: Path) -> np.ndarray:
        ...
        if self._cache is not None:
            ...
            if cached is not None:
                return cached.copy()  # Return copy to avoid mutations
        ...
        *,
        ...
        for start in range(0, len(indices), batch_size):
            ...
            for idx in batch_indices:
                sample = samples[idx]
                image = self._load_image_cached(sample.path)
    ...
    *,
    ...
    for sample in samples:
        if image_cache is not None:
            image = image_cache.get(sample.path)
            if image is None:
...
EmotionDataModule = EmotionDataModuleOptimized
