"""Optimized training loops with JIT compilation and efficient data loading."""
...
def _make_train_step_fn(
    ...
    """Create the core training step function that can be JIT compiled."""
    ...
    def train_step_inner(
        ...
        """Inner training step that operates on pure JAX arrays."""

        def loss_fn(params):
            logits = model_apply_fn(params, images)
            ...
            return loss, {"loss": loss, "accuracy": accuracy, "logits": logits}
    ...
    return train_step_inner


def _make_eval_step_fn(label_smoothing: float, use_mixed_precision: bool):
    """Create the core eval step function that can be JIT compiled."""
    ...
    def eval_step_inner(
        ...
        """Inner eval step that operates on pure JAX arrays."""
        logits = model_apply_fn(params, images)
        loss = cross_entropy_loss(
            logits,
            labels,
...
def train_and_evaluate(config: TrainingConfig, mesh=None) -> TrainingSummary:
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if config.log_to_console:
            print(
                f"[epoch {epoch:03d}] train_loss={epoch_train_loss:.4f} "
