"""Speed-optimized training with JIT compilation, prefetching, and Mixup.

Key optimizations:
1. JIT-compiled train and eval steps
2. Data prefetching to overlap CPU/GPU work
3. Efficient mixup implementation
...
# =============================================================================
# DATA PREFETCHER - Overlaps data loading with GPU computation
# =============================================================================
...
class TrainingConfig:
    """Training configuration with mixup support."""
...
# =============================================================================
# MIXUP - Efficient implementation
# =============================================================================
...
def create_lr_schedule(config: TrainingConfig, steps_per_epoch: int) -> optax.Schedule:
    ...
    def schedule(step):
        # Warmup phase
        warmup_lr = config.learning_rate * step / max(warmup_steps, 1)
        
        # Cosine decay phase
        progress = (step - warmup_steps) / max(total_steps - warmup_steps, 1)
        cosine_lr = config.min_learning_rate + 0.5 * (config.learning_rate - config.min_learning_rate) * (1 + jnp.cos(jnp.pi * progress))
...
def make_train_step(config: TrainingConfig, num_classes: int):
    ...
    def train_step(state: TrainState, images: jnp.ndarray, labels: jnp.ndarray, rng: jax.random.PRNGKey):
        # Cast to FP16 if mixed precision
...
def train_and_evaluate(config: TrainingConfig) -> TrainingSummary:
    """Main training function with speed optimizations."""
    ...
    print(f"Batch size: {config.batch_size} x {num_devices} GPUs = {config.batch_size * num_devices}")
    ...
    for epoch in range(1, config.num_epochs + 1):
        ...
        train_iter = data_module.train_batches(rng_seed=epoch_seed, batch_size=config.batch_size)
        ...
        for step, (images, labels) in enumerate(prefetcher, 1):
            ...
            if step % config.log_every == 0:
                ...
                print(f"  E{epoch:02d} S{step:04d}/{steps_per_epoch} | loss={avg_loss:.4f} acc={avg_acc*100:.1f}% | {steps_sec:.1f} steps/s", flush=True)
        ...
        for images, labels in data_module.val_batches(batch_size=config.batch_size):
            ...
            val_labels.append(np.asarray(labels))
    ...
    for images, labels in data_module.test_batches(batch_size=config.batch_size):
        ...
        test_labels.append(np.asarray(labels))
    
    test_preds = np.concatenate(test_preds)
    test_labels = np.concatenate(test_labels)
    
    test_acc = np.mean(test_preds == test_labels)
    test_cm = compute_confusion_matrix(test_preds, test_labels, num_classes)
